...
void NGLScene::initializeGL()
{
  ...
  auto scale=std::uniform_int_distribution<int>(0,6);
  ngl::Random::addIntGenerator("particleScale",scale);
  ...
  startTimer(10); //Time in Miliseconds 100fps
}
...
void NGLScene::timerEvent(QTimerEvent *_event)
{
  ...
  m_emitter->update();//everytime the time triggers, we call emitter to update.
...
void NGLScene::paintGL()
{
  ...
  auto text=fmt::format("Draw took {} uS", std::chrono::duration_cast<std::chrono::microseconds>(renderEnd-renderBegin).count());
  m_text->renderText(10,680,text);
  m_text->renderText(10,660,fmt::format("Update took {}",m_updateTime));
